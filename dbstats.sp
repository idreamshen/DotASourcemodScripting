...
public Action:collect_gameplay_data(Handle:timer) {
	...
	new now = GetTime();
	new Handle:hQuery;
	decl String:strHostname[4096];
	GetConVarString(FindConVar("hostname"), strHostname, sizeof(strHostname));
	
	if((ingame_clients - fake_clients))
	{
		total_conn_time = total_conn_time / (ingame_clients - fake_clients);
	}
	
	Format(query, sizeof(query), "SELECT ip FROM gameplay WHERE ip = '%s' AND port = '%s'", server_ip, server_port);
	hQuery = SQL_Query(database, query);
	if(SQL_GetRowCount(hQuery))
	{
		Format(query, sizeof(query), "UPDATE gameplay SET time = %d, clients = %d, ingame_clients = %d, bots = %d, kills = %d, bot_kills = %d, deaths = %d, connected_mins = %f WHERE ip = '%s' AND port = '%s'",
		now, clients, ingame_clients, fake_clients,
		total_kills, fake_kills, total_deaths, total_conn_time / 60.0, server_ip, server_port);
		SQL_TQuery(database, validate_insert_gameplay, query, now);
		
	}
	else 
	{
		Format(query, sizeof(query), "INSERT INTO gameplay (ip, port, time, clients, ingame_clients, bots, kills, bot_kills, deaths, connected_mins, host_name) VALUES ('%s', %s, %d, %d, %d, %d, %d, %d, %d, %f, '%s')",
		server_ip, server_port, now, clients, ingame_clients, fake_clients,
		total_kills, fake_kills, total_deaths, total_conn_time / 60.0 , strHostname);
		SQL_TQuery(database, validate_insert_gameplay, query, now);
	}
	CloseHandle(hQuery);
...
public Action:collect_teams_data(Handle:timer) {
	...
	for (new team=0; team<num_teams; ++team) {
		...
		new now = GetTime();
		Format(query, sizeof(query), "INSERT INTO teams (ip, port, time, id, name, score, ingame_clients, bots, kills, bot_kills, deaths, connected_mins) VALUES(%s, %s, %d, %d, '%s', %d, %d, %d, %d, %d, %d, %f)",
